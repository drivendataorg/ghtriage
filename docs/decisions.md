...
its `.gitignore` is deleted by hand and regenerated by the next command — acceptable because the
tool had no users besides its author when the change landed, and cheaper than a rewrite path that
has to decide whether a hand-edited ignore file is stale. The stale whitelist exposes only
`config.toml`; the leading `*` keeps ignoring the token.
...
front of the reader. Every value is commented out — only the inert `[auth]` table header is
live — so a fresh scaffold parses as an empty config and changes nothing.
...
targets the wrong repository with no signal. Unknown keys and tables warn on stderr. Type errors
